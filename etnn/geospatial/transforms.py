...
def add_virtual_node(data: CombinatorialComplexData) -> CombinatorialComplexData:
    ...
    data.cells_3 = torch.cat(data.cell_list(rank=0)).to(data.pos.device)
    data.slices_3 = torch.tensor([len(data.cells_3)]).to(data.pos.device)

    # connect every two cell
    # num_cells_2 = len(data.slices_2)
    num_cells_2 = len(data.cell_list(rank=2))
